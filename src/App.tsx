...
export function get_name_and_font(
  pptx_elem: Element,
  index: number
): ObjInfo | null {
  let name = pptx_elem.getElementsByTagNameNS(
    "http://schemas.openxmlformats.org/presentationml/2006/main",
    "nvSpPr"
  )[0];
  name = name.getElementsByTagNameNS(
    "http://schemas.openxmlformats.org/presentationml/2006/main",
    "cNvPr"
  )[0];
  ...
  let font_elem = pptx_elem.getElementsByTagNameNS(
    "http://schemas.openxmlformats.org/presentationml/2006/main",
    "txBody"
  )[0];
  font_elem = font_elem.getElementsByTagNameNS(
  ...
  let fontname_elem = font_elem.getElementsByTagNameNS(
    "http://schemas.openxmlformats.org/drawingml/2006/main",
    "latin"
  )[0];
  let fontcolor_elem: Element | null = font_elem.getElementsByTagNameNS(
    "http://schemas.openxmlformats.org/drawingml/2006/main",
    "solidFill"
  )[0];
  ...
  if (!fontcolor_elem) {
    ...
    let srgbClr: Element | null = fontcolor_elem.getElementsByTagNameNS(
      "http://schemas.openxmlformats.org/drawingml/2006/main",
      "srgbClr"
    )[0];
    if (srgbClr) {
...
type appstate = {
  fonts: JSX.Element[];
  font_check_result: boolean | null;
  show_fonts: boolean;
  finish: boolean;
  error: boolean;
...
class App extends Component<any, appstate> {
  constructor(props: any) {
    ...
    this.state = {
      fonts: [],
      font_check_result: null,
      show_fonts: false,
      finish: false,
      color_check_result: null,
      error: true,
    };
  }
  ...
  showFile = async (e: any) => {
    ...
    reader.onload = async (e1) => {
      ...
      t = await zip
        .loadAsync(e1.target.result)
        .then(async (e: any) =>
          Object.keys(e.files)
            .filter((a) => a.startsWith("ppt/slides/slide"))
            .map((e1) => e.files[e1])
      ...
      t.forEach((page, index) => {
        page.then((page_: string) => {
          ...
          let data_array = Array.from(
            data.getElementsByTagNameNS(
              "http://schemas.openxmlformats.org/presentationml/2006/main",
              "sp"
            )
          );
      ...
      Promise.all(t).then(async () => {
        ...
        new Promise<JSX.Element[]>((res) => {
          ...
          if (shapes.every((val, i, arr) => val["font"] === arr[0]["font"])) {
            ...
            this.setState((prev) => ({
              ...prev,
              font_check_result: false,
              finish: true,
              fonts: fonts,
              error: false,
            }));
          }
          let color_count = new Set(shapes.map((shape) => shape["font_color"]))
            .size;
    ...
    return (
      <div className="container">
        ...
        <div className="description">
          <p>
            It examines the pptx file you upload and extracts the fonts used in
            it. Currently, it only can inspect font information and the results
            of the analysis may not be correct.
          </p>
        </div>
        <p>
          <b>
            {" "}
            The analysis is done in the browser. This website doesn't save your
            file to servers.
          </b>
        </p>
          ...
          <div className="results">
                ...
                {this.state.color_check_result ? (
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <VscPass
                      ...
                      color={"#008000"}
                    />{" "}
                    The number of the font colors is below 4.{" "}
                  </div>
                ) : (
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <VscError
                      ...
                      color={"#ff0000"}
                    />{" "}
                    The number of the font colors is above 4.{" "}
                  </div>
                )}
              </div>
            </div>
            <button
              onClick={() =>
                this.setState((prev) => ({
                  ...prev,
                  show_fonts: prev.show_fonts ? false : true,
                }))
              }
            >
              {this.state.show_fonts ? "- Hide Detail" : "+ Show Detail"}
            </button>
        ...
        <footer>
          <p>
            © Tomohiro Endo
            <address>
              <a href="https://github.com/europeanplaice/pptx_checker_ts">
                https://github.com/europeanplaice/pptx_checker_ts
              </a>
            </address>
          </p>
        </footer>
